...
var qrcode1 = "1";
//右侧悬浮二维码，1自动生成（当前页面二维码），填写路径将加载指定图片（建议尺寸150*150px）例：/Public/97kpw/img/wxcode.png

var qrcode2 = "1";
//内容页二维码，1自动生成（当前页面二维码），填写路径将加载指定图片（建议尺寸160*160px）例：/Public/97kpw/img/wxcode.png

var copyshort = "1"
//是否开启自动生成短链接，0关闭1开启（开启后点击分享按钮将会复制短链否则复制默认链接）

var baidushare = "/Public/97kpw/api/js/share.js";
//是否开启百度分享，0关闭，启用本地分享填写/Public/97kpw/api/js/share.js（支持https），启用官网分享填写http://bdimg.share.baidu.com/static/api/js/share.js（不支持https）
...
var stui = {
	...
	$(".play-tab li").on('click',function(){
			    ...
			    $(this).parent().parent().find("h3 a").html($(this).html())
				$(".play-tab").hide();	
	  	});
